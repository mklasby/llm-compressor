...
from llmcompressor.modifiers.obcq.sgpt_tiled_sparsify import (
    accumulate_hessian,
    make_empty_hessian,
    sparsify_weight,
)
...
__all__ = ["SparseGPTTiledModifier"]


class SparseGPTTiledModifier(SparsityModifierMixin, Modifier):
    """
    Modifier for applying the one-shot SparseGPT algorithm to a model with CS256 sparse format.
    ...
    _module_names: Dict[torch.nn.Module, str] = PrivateAttr(
        default_factory=dict
    )
    _module_sparsities: Dict[torch.nn.Module, str] = PrivateAttr(
        default_factory=dict
    )
